...
class HyperparameterOptimizer:
    ...
    def generate_parameters(self) -> List[Namespace]:
        ...
        model_params = [7, 5, 10,13]  # important
        reject_dis_values = [0.02, 0.01]
        attract_point_comps = [0.1,0.05]
        ori_coef = [0.001, .1, 0.05]
        low_err_coer = [0.3, 0.6]
        upper_err_coef = [0.9, 1]
    ...
    def _evaluate_single_trial(self, params: Namespace, trial_count: int) -> Dict:
        ...
        try:
            ...
            while rollout_step < max_rollout_length:
                ...
                p_arm_local, x_plus_opt, error, info = param.lambda_optimizer.optimize_control_input(
                    target_pose_local, current_x, gravity, local_point)
                p_arm_world = R_obj_to_world @ p_arm_local + curr_q[:3]

   
                local_point = R_obj_to_world.T @ (curr_q[7:10] - curr_q[0:3])
                p_arm_local, normal, x_plus_opt, error, info = param.lambda_optimizer.optimize_control_input(target_pose_local, current_x, gravity, local_point)
                p_arm_world = R_obj_to_world @ p_arm_local + curr_q[:3]

                pos_err = cs.norm_2(param.target_p_[:2] - curr_q[:2])
                ori_err = 1 - cs.dot(curr_q[3:7], param.target_q_) ** 2
                curr_err_ref = params.pos_coef * pos_err + params.ori_coef * ori_err

                eps = 1e-6
                delta_error = max_error - min_error
                delta_error = max(delta_error, eps)

                # 当前 error 在 [min_error, max_error] 区间中的归一化位置
                # norm_err ∈ [0,1]，越小越好（接近 min_error）
                ...
                if verify_cost:
                    upper_err_coef = params.upper_err_coef
                elif np.linalg.norm(curr_q[7:10] - attract_point_world) < 5e-2:
                        upper_err_coef *= 0.9
                # low_err_coef = args.low_err_coef if not verify_cost else max(low_err_coef+0.002, 0.7)
                ...
                if not verify_cost and consecutive_detect_time >= 10:
                    verify_cost = 1
                elif verify_cost and consecutive_detect_time >= 5 and consecutive_contact_time >= 15:
                    verify_cost = 0
