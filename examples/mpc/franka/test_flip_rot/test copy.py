...
DEFAULT_TRAJ_OUTPUT_DIR = "/home/lab423/scsp/scsp-robot/outputs/traj_record"
...
def _filter_point_indices_to_upper_third(optimizer, pos, rotation_matrix, candidate_idx):
    candidate_idx = np.asarray(candidate_idx, dtype=np.int32).reshape(-1)
    if candidate_idx.size == 0:
        return candidate_idx

    # Keep the legacy function name for compatibility, but filter by the
    # lower half of the candidate points in the current world-frame z axis.
    sample_points = np.asarray(optimizer.sample_point, dtype=np.float32)
    centers_world = (np.asarray(rotation_matrix, dtype=np.float32) @ sample_points[candidate_idx].T).T + np.asarray(
        pos,
        dtype=np.float32,
    )
    z_coords = centers_world[:, 2]
    z_span = float(np.max(z_coords) - np.min(z_coords))
    if z_span <= 1e-6:
        return candidate_idx

    z_cutoff = float(np.min(z_coords) + 0.5 * z_span)
    lower_mask = z_coords <= z_cutoff
    lower_idx = candidate_idx[lower_mask]
    return lower_idx if lower_idx.size > 0 else candidate_idx
...
def main():
    ...
    parser.add_argument("--attract_point_comp", type=float, default=0.07, help="distance compensation of attract point")
    ...
    parser.add_argument(
        "--video-output-dir",
        type=str,
        default="/home/lab423/scsp/scsp-robot/outputs/videos_panda",
        help="Directory to save Isaac Gym MP4 videos. Leave empty to disable recording.",
    )
    parser.add_argument(
        "--video-fps",
        type=float,
    ...
    parser.add_argument(
        "--traj-output-dir",
        type=str,
        default=DEFAULT_TRAJ_OUTPUT_DIR,
        help="Directory to save per-trial object-pose trajectories as JSON. Leave empty to disable recording.",
    )
    parser.add_argument(
        "--print-object-pose",
        type=_parse_bool_arg,
    ...
    for trial_count in range(trial_start, trial_stop):
        ...
        param.video_output_path_ = (
            os.path.join(args.video_output_dir, f"trial_{trial_count:03d}.mp4")
            if args.video_output_dir
            else None
        )
        traj_output_path = (
            os.path.join(args.traj_output_dir, f"trial_{trial_count:03d}.json")
            if args.traj_output_dir
            else None
        )
        ...
        def _save_traj_snapshot():
            nonlocal saved_traj_path
            ...
            if saved_traj_path is not None:
                print(f"[trial {trial_count:03d}] saved traj json: {saved_traj_path}")
            return saved_traj_path
        ...
        try:
            ...
            while rollout_step < max_rollout_length and not env.break_out_signal_:
                    if not env.dyn_paused_:
                        ...
                        plan_u_lb = -0.02 if plan_like_ik2 else -0.06
                        plan_u_ub = 0.02 if plan_like_ik2 else 0.06
                        sol = mpc.plan_once(
                            effective_target_p,
                            param.target_q_,
                            curr_x_solver,
                        ...
                        if best_contact_torque_world is not None:
                            best_contact_torque_scale = 0.1 if best_contact_torque_scale_flag else 1.0
