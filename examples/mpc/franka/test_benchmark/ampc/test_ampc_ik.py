...
"""
    ...
    def step_passive(self):
        ...
        self.gym.set_dof_actuation_force_tensor(self.sim, gymtorch.unwrap_tensor(self._effort_control))
        self._simulate_once()
        return np.zeros((int(getattr(self.param_, "n_cmd_", 3)),), dtype=np.float32)
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="blend between center attraction and surface-proxy attraction for the 3D point action (0=surface proxy, 1=center)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="desired approach standoff from the object center for the surface proxy (m); default estimates from mesh size",
    )
    parser.add_argument(
        "--point-path-cost-weight",
        type=float,
        default=60.0,
        help="path-stage weight that keeps the 3D point moving toward the approach/contact proxy",
    )
    parser.add_argument(
        "--point-final-cost-weight",
        type=float,
        default=400.0,
        help="terminal weight that keeps the optimizer from stopping early before contact",
    )
    parser.add_argument(
        "--point-control-cost-weight",
        type=float,
        default=0.1,
        help="quadratic penalty on 3D point actions",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="bound of each 3D point displacement action per MPC step (m)",
    )
    parser.add_argument(
    ...
    parser.add_argument("--trial-start", type=int, default=2, help="first trial id / random seed to evaluate")
    ...
    if not (0.0 <= float(args.contact_cost_param) <= 1.0):
        raise ValueError(f"contact_cost_param must be in [0, 1], got {args.contact_cost_param}")
    if args.approach_offset is not None and args.approach_offset < 0.0:
        raise ValueError(f"approach_offset must be non-negative, got {args.approach_offset}")
    if args.point_path_cost_weight < 0.0:
        raise ValueError(f"point_path_cost_weight must be non-negative, got {args.point_path_cost_weight}")
    if args.point_final_cost_weight < 0.0:
        raise ValueError(f"point_final_cost_weight must be non-negative, got {args.point_final_cost_weight}")
    if args.point_control_cost_weight < 0.0:
        raise ValueError(f"point_control_cost_weight must be non-negative, got {args.point_control_cost_weight}")
        ...
        try:
            ...
            while rollout_step < max_rollout_length and not env.break_out_signal_:
                if not env.dyn_paused_:
                    ...
                    curr_x_solver = np.hstack([curr_q[:7], ee_pos_world]).astype(np.float32)
                    sol = mpc.plan_once(
                        param.target_p_,
                        param.target_q_,
                        curr_x_solver,
                    ...
                    env.step(action, desired_force_world=None, desired_torque_world=None)
                    rollout_step += 1
