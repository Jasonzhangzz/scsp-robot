...
from examples.mpc.franka.test_benchmark.ampc.params_ampc import ExplicitMPCParams
from examples.mpc.franka.ik2.test_mppi_isaac_for_cimpc import (
    IsaacFrankaSimulator,
    _parse_bool_arg,
    _extract_quat_xyzw,
    _extract_vec3,
)
from examples.mpc.franka.test_benchmark.cimpc import MPCExplicit
...
DEFAULT_RECORD_OUTPUT_DIR = "/home/lab423/scsp/scsp-robot/outputs/videos_ampc"
...
def _quat_wxyz_to_matrix(quat_wxyz):
    quat_xyzw = _quat_wxyz_to_xyzw_np(quat_wxyz)
    return Rotation.from_quat(np.asarray(quat_xyzw, dtype=np.float64)).as_matrix().astype(np.float32)
...
def _load_object_geometry_cache(mesh_path, repo_root=REPO_ROOT):
    try:
        import trimesh
    except ImportError as exc:
        raise ImportError("Stage-1 OSC approach in test_ampc.py requires trimesh to load the object mesh.") from exc

    if mesh_path is None:
        raise ValueError("mesh_path is required to build the stage-1 object geometry cache")

    candidate_mesh_paths = [mesh_path] if os.path.isabs(mesh_path) else [
        os.path.join(repo_root, mesh_path),
        os.path.join(os.getcwd(), mesh_path),
        os.path.abspath(mesh_path),
    ]
    mesh_abs_path = None
    for candidate_path in candidate_mesh_paths:
        candidate_path = os.path.abspath(candidate_path)
        if os.path.isfile(candidate_path):
            mesh_abs_path = candidate_path
            break
    if mesh_abs_path is None:
        raise FileNotFoundError(
            f"Object mesh not found. Tried: {', '.join(os.path.abspath(path) for path in candidate_mesh_paths)}"
        )
    ...
    if vertices.ndim != 2 or vertices.shape[0] == 0 or vertices.shape[1] != 3:
        raise ValueError(f"Object mesh has invalid vertices: {mesh_abs_path}")

    geometric_center_local = np.asarray(mesh.bounds, dtype=np.float32).mean(axis=0)
    surface_tree = cKDTree(np.asarray(vertices, dtype=np.float64))
    return geometric_center_local.astype(np.float32), surface_tree, mesh_abs_path, int(vertices.shape[0])


def _transform_point_from_object_local(local_point, obj_pos_world, obj_rotation_world):
    local_point = np.asarray(local_point, dtype=np.float32).reshape(3)
    obj_pos_world = np.asarray(obj_pos_world, dtype=np.float32).reshape(3)
    obj_rotation_world = np.asarray(obj_rotation_world, dtype=np.float32).reshape(3, 3)
    return (obj_rotation_world @ local_point + obj_pos_world).astype(np.float32)


def _compute_fingertip_object_surface_gap(ee_pos_world, obj_pos_world, obj_rotation_world, object_surface_tree):
    ee_pos_world = np.asarray(ee_pos_world, dtype=np.float32).reshape(3)
    obj_pos_world = np.asarray(obj_pos_world, dtype=np.float32).reshape(3)
    obj_rotation_world = np.asarray(obj_rotation_world, dtype=np.float32).reshape(3, 3)
    ee_pos_local = obj_rotation_world.T @ (ee_pos_world - obj_pos_world)
    nearest_surface_distance, _ = object_surface_tree.query(np.asarray(ee_pos_local, dtype=np.float64), k=1)
    return float(max(float(nearest_surface_distance) - FINGERTIP_RADIUS, 0.0))
...
def _apply_dywa_physics_to_param(param):
    ...
    # Keep the stage-2 solver/physics parameters aligned with
    # examples/mpc/franka/direct_control/params.py instead of re-randomizing
    # them per trial.
    param.table_friction_ = 0.5
    param.object_friction_ = float(param.mu_object_)
    param.fingertip_friction_ = float(param.mu_object_)
    param.obj_mass_ = float(param.obj_mass_)
    param.gravity_[2] = -9.8
    return param
...
class IsaacFrankaOSCSimulator(IsaacFrankaSimulator):
    def __init__(self, param, headless=False, sim_device="cuda:0", graphics_device_id=0):
        ...
        gravity_vec = np.asarray(
            getattr(self.param_, "gravity_", np.array([0.0, 0.0, -9.8], dtype=np.float32)),
            dtype=np.float32,
        ).reshape(-1)
        gravity_z = float(gravity_vec[2])
        sim_params.gravity = gymapi.Vec3(0.0, 0.0, gravity_z)
        ...
        sim_params.physx.bounce_threshold_velocity = 2.0 * abs(gravity_z) * self.sim_dt_ / max(sim_substeps, 1)
...
"""
    ...
    def step_pose_target(
        ...
        preserve_nullspace_target=False,
        desired_force_world=None,
        desired_torque_world=None,
    ):
        self.p_d = np.asarray(target_position, dtype=np.float32).reshape(3).copy()
        if target_rotation is None:
            target_rotation = self.R_d_hold.copy()
        target_rotation = self._project_to_rotation_matrix(np.asarray(target_rotation, dtype=np.float32).reshape(3, 3))
        self.R_d = target_rotation.copy()
        self.R_d_hold = target_rotation.copy()
        self.set_tool_compensation_wrench(
            force_world=desired_force_world,
            torque_world=desired_torque_world,
        )
        return self._track_desired_pose(preserve_nullspace_target=preserve_nullspace_target)
    ...
    def detect_once(self, simulator: IsaacFrankaOSCSimulator):
        ...
        for c in contacts:
            ...
            if other_sim_idx in simulator.franka_body_indices:
                j_other[:, 6:] = simulator.get_body_point_jacobian(other_sim_idx, cpos)
        ...
        if row_env_idx == 0:
            ...
            if dist_table < 0.02:
                ...
                j_rel_table = np.concatenate([np.eye(3), np.zeros((3, nv - 3))], axis=1)
...
def _run_trials_for_object(args, trial_count_overridden):
    ...
    success_rate = 0
    cached_object_mesh_key = None
    object_geometric_center_local = None
    object_surface_tree = None
    ...
    for experiment_idx, trial_spec in enumerate(trial_specs):
        ...
        if cached_object_mesh_key != param.mesh_path_:
            (
                object_geometric_center_local,
                object_surface_tree,
                loaded_mesh_path,
                loaded_mesh_vertex_count,
            ) = _load_object_geometry_cache(param.mesh_path_)
            cached_object_mesh_key = param.mesh_path_
            print(
                f"Loaded object geometry from {loaded_mesh_path}: "
                f"vertices={loaded_mesh_vertex_count}, "
                f"center_local={_format_array_for_print(object_geometric_center_local)}"
            )
        ...
        try:
            ...
            rollout_step = 0
            use_joint_mpc_stage = False
            stage1_target_rotation = env.get_end_effector_pos()[1].copy()
            ...
            while rollout_step < max_rollout_length and not env.break_out_signal_:
                if not env.dyn_paused_:
                    ...
                    (
                        ...
                        _con_point,
                        _jac_mat_env,
                        _robot_contact_force_map,
                        _if_contact,
                        _if_fingertip_contact,
                    ...
                    if (
                        ...
                        print(
                            ...
                            f"stage={'ampc' if use_joint_mpc_stage else 'approach'}"
                        )
                    ...
                    obj_pos_world = np.asarray(curr_q[:3], dtype=np.float32).copy()
                    obj_quat_wxyz = np.asarray(curr_q[3:7], dtype=np.float32).copy()
                    obj_rotation_world = _quat_wxyz_to_matrix(obj_quat_wxyz)
                    ee_pos_world, _ = env.get_end_effector_pos()
                    object_center_world = _transform_point_from_object_local(
                        object_geometric_center_local,
                        obj_pos_world,
                        obj_rotation_world,
                    )
                    fingertip_surface_gap = _compute_fingertip_object_surface_gap(
                        ee_pos_world,
                        obj_pos_world,
                        obj_rotation_world,
                        object_surface_tree,
                    ...
                    if (not use_joint_mpc_stage) and (
                        fingertip_surface_gap <= float(args.stage1_switch_distance)
                        or (bool(args.stage1_switch_on_contact) and _if_fingertip_contact)
                    ):
                        use_joint_mpc_stage = True
                        param.sol_guess_ = None
                        print(
                            f"[trial {trial_count:03d} step {rollout_step:04d}] "
                            "switching to AMPC joint-delta stage: "
                            f"surface_gap={fingertip_surface_gap:.6f}, "
                            f"fingertip_contact={bool(_if_fingertip_contact)}"
                        )

                    env.apply_object_wrench_world(force_world=None, torque_world=None)
                    if use_joint_mpc_stage:
                        curr_x_solver = np.asarray(curr_q, dtype=np.float32).reshape(param.n_qpos_)
                        sol = mpc.plan_once(
                            param.target_p_,
                            param.target_q_,
                            curr_x_solver,
                        ...
                        action = np.asarray(sol["action"], dtype=np.float32).reshape(param.n_cmd_)
                        env.step_joint_delta(action)
                    else:
                        env.step_pose_target(
                            target_position=object_center_world,
                            target_rotation=stage1_target_rotation,
                            preserve_nullspace_target=False,
                            desired_force_world=None,
                            desired_torque_world=None,
                        )
...
def main():
    ...
    parser.add_argument("--model_param", type=float, default=10, help="model param")
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="bound of each joint displacement action per MPC step (rad)",
    )
    parser.add_argument(
        "--joint-action-stiffness",
        "--cartesian-joint-stiffness",
        dest="joint_action_stiffness",
        type=float,
        default=200.0,
        help="internal joint-action stiffness used in the explicit model",
    )
    ...
    parser.add_argument(
        "--trails-input-root",
        type=str,
        default=DEFAULT_TRAILS_INPUT_ROOT,
        help="Root directory containing per-object trials.json/trails.json files used to set init/goal poses.",
    )
    parser.add_argument(
        "--print-object-pose",
        type=_parse_bool_arg,
    ...
    parser.add_argument(
        "--small-action-break-min-video-seconds",
        type=float,
        default=60.0,
        help="If MP4 recording has lasted at least this many real-time seconds and the AMPC action is tiny, stop the current trial early.",
    )
    parser.add_argument(
        "--small-action-break-threshold",
        type=float,
        default=1e-3,
        help="Infinity-norm threshold below which the AMPC joint-delta action is treated as tiny for early stopping.",
    )
    parser.add_argument(
        "--stage1-switch-distance",
        type=float,
        default=0.008,
        help="Switch from the OSC approach stage to AMPC once the fingertip-to-object surface gap falls below this threshold (m).",
    )
    parser.add_argument(
        "--stage1-switch-on-contact",
        type=_parse_bool_arg,
        default=True,
        help="If true, switch to AMPC immediately once PhysX reports fingertip-object contact.",
    )
    ...
    if args.small_action_break_threshold < 0.0:
        raise ValueError(
            f"small_action_break_threshold must be non-negative, got {args.small_action_break_threshold}"
        )
    if args.stage1_switch_distance < 0.0:
        raise ValueError(f"stage1_switch_distance must be non-negative, got {args.stage1_switch_distance}")
