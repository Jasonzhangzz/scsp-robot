...
from examples.mpc.franka.test_benchmark.sampling.params_sampling import ExplicitMPCParams
...
from planning.MPCExplicit_tilted_push import MPCExplicitIsaac
...
DEFAULT_RECORD_OUTPUT_DIR = "/home/lab423/scsp/scsp-robot/outputs/videos_sampling"
DEFAULT_TRIALS_INPUT_ROOT = "/home/lab423/scsp/scsp-robot/outputs/videos_panda"
DEFAULT_TRAILS_INPUT_ROOT = DEFAULT_TRIALS_INPUT_ROOT
TRIAL_SPECS_FILENAMES = ("trials.json", "trails.json")
...
def _resolve_trial_specs_path(obj_name, trails_root):
    trails_root = os.path.abspath(trails_root)
    candidate_paths = [
        os.path.join(trails_root, str(obj_name), file_name)
        for file_name in TRIAL_SPECS_FILENAMES
    ]
    for candidate_path in candidate_paths:
        if os.path.isfile(candidate_path):
            return candidate_path
    raise FileNotFoundError(
        f"Missing trials/trails json for obj '{obj_name}'. "
        f"Tried: {', '.join(candidate_paths)}"
    )


def _discover_object_names(trails_root):
    trails_root = os.path.abspath(trails_root)
    if not os.path.isdir(trails_root):
        raise NotADirectoryError(f"trails_input_root is not a directory: {trails_root}")

    object_names = []
    skipped_names = []
    for entry in sorted(os.scandir(trails_root), key=lambda item: item.name):
        if not entry.is_dir():
            continue
        try:
            _resolve_trial_specs_path(entry.name, trails_root)
    ...
    if not object_names:
        raise FileNotFoundError(
            f"No object directories with trials.json/trails.json were found under {trails_root}"
        )

    if skipped_names:
        print(
            f"Skipping {len(skipped_names)} object directories without trials.json/trails.json "
            f"under {trails_root}: {', '.join(skipped_names)}"
        )

    return object_names


def _normalize_object_names(obj_arg, trails_root):
    if obj_arg is None:
        return _discover_object_names(trails_root)

    if isinstance(obj_arg, str):
        normalized = [obj_arg]
    else:
        normalized = [str(name) for name in obj_arg if str(name).strip()]

    normalized = [name.strip() for name in normalized if name.strip()]
    return normalized if normalized else _discover_object_names(trails_root)


def _load_trial_specs_from_trails(obj_name, trails_root):
    trails_path = _resolve_trial_specs_path(obj_name, trails_root)

    with open(trails_path, "r", encoding="utf-8") as f:
        payload = json.load(f)

    if not isinstance(payload, list) or len(payload) == 0:
        raise ValueError(f"Expected a non-empty list in {trails_path}")

    trial_specs = []
    for idx, entry in enumerate(payload):
        if not isinstance(entry, dict):
            raise ValueError(f"Invalid trail entry at index {idx} in {trails_path}: expected object")

        init_xyz = np.asarray(entry["init_xyz"], dtype=np.float32).reshape(3)
        init_quat_wxyz = np.asarray(entry["init_quat_wxyz"], dtype=np.float32).reshape(4)
        goal_xyz = np.asarray(entry["goal_xyz"], dtype=np.float32).reshape(3)
        goal_quat_wxyz = np.asarray(entry["goal_quat_wxyz"], dtype=np.float32).reshape(4)

        init_quat_wxyz = init_quat_wxyz / max(np.linalg.norm(init_quat_wxyz), 1e-8)
        goal_quat_wxyz = goal_quat_wxyz / max(np.linalg.norm(goal_quat_wxyz), 1e-8)

        trial_specs.append(
            {
                "source_trial_id": int(entry.get("trial", idx)),
                "init_xyz": init_xyz.copy(),
                "init_xy_rand": init_xyz[:2].copy(),
                "init_obj_quat_rand": init_quat_wxyz.copy(),
                "target_p": goal_xyz.copy(),
                "target_q": goal_quat_wxyz.copy(),
            }
        )

    return trial_specs, trails_path
...
def _run_trials_for_object(args, trial_count_overridden):
    trial_start = int(args.trial_start)
    all_trial_specs, trails_path = _load_trial_specs_from_trails(args.obj, args.trails_input_root)
    if trial_start >= len(all_trial_specs):
        raise ValueError(
            f"trial_start={trial_start} is out of range for {len(all_trial_specs)} trial specs loaded from {trails_path}"
        )
    if trial_count_overridden:
        trial_specs = all_trial_specs[trial_start : trial_start + int(args.trial_count)]
    else:
        trial_specs = all_trial_specs[trial_start:]

    trial_num = len(trial_specs)
    max_rollout_length = 5000
    success_rate = 0
    object_interrupted = False
    print(f"[obj {args.obj}] Loaded {len(all_trial_specs)} trial specs from {trails_path}")

    for experiment_idx, trial_spec in enumerate(trial_specs):
        trial_count = experiment_idx
        source_trial_id = int(trial_spec["source_trial_id"])
        args.init_xyz = np.asarray(trial_spec["init_xyz"], dtype=np.float32).copy()
        args.init_xy_rand = np.asarray(trial_spec["init_xy_rand"], dtype=np.float32).copy()
        args.init_obj_quat_rand = np.asarray(trial_spec["init_obj_quat_rand"], dtype=np.float32).copy()
        args.target_p = np.asarray(trial_spec["target_p"], dtype=np.float32).copy()
        args.target_q = np.asarray(trial_spec["target_q"], dtype=np.float32).copy()
        ...
        print(
            f"[trial {trial_count:03d}] source_trial={source_trial_id} "
            f"obj={args.obj} "
            f"init_seed={int(param.random_seed_)} "
        ...
        try:
            ...
            current_x[3] = 1.0
            video_recording_start_wall_time = time.perf_counter() if env.video_recorder_ is not None else None
            ...
            while rollout_step < max_rollout_length and not env.break_out_signal_:
                if not env.dyn_paused_:
                    ...
                    current_recorded_video_seconds = (
                        time.perf_counter() - video_recording_start_wall_time
                        if video_recording_start_wall_time is not None
                        else 0.0
                    )
                    if (
                        env.video_recorder_ is not None
                        and current_recorded_video_seconds >= float(args.small_action_break_min_video_seconds)
                    ):
                        env.break_out_signal_ = True
                        saved_video_path = env.finalize_video_recording()
                        print(
                            f"[trial {trial_count:03d} step {rollout_step:04d}] "
                            "stopping early: mp4 real-time duration exceeded threshold. "
                            f"video_elapsed={current_recorded_video_seconds:.2f}s"
                        )
                    ...
                    v_obj_world_raw = np.hstack([obj_linear_velocity_world, obj_angular_velocity_world]).astype(
                        np.float32
                    )
                    ...
                    attract_point_world = _compute_safe_attract_point_world(
                        contact_point_local=best_contact_point.copy(),
                        normal_local=normal,
                    ...
                    p_arm_local, _, x_plus_opt, error, info, p_arm_torque_world = (
                        param.lambda_optimizer.optimize_control_input(
                            target_pose_local,
                            current_x,
                            gravity,
                            local_point,
                    ...
                    delta_error = max(max_error - min_error, 1e-6)
                    rel_impr = max_error - error
                    impr_th = max_error - min_error
                    improvement = 0 if impr_th == 0 else rel_impr / impr_th
                    ...
                    action = np.asarray(sol["action"], dtype=np.float32).copy()
                    ...
                    if pos_error < float(args.success_pos_threshold) and quat_error < float(
                        args.success_quat_threshold
                    ):
                        trial_success = True
                        saved_video_path = env.finalize_video_recording()
                        print(
    ...
    success_ratio = (success_rate / trial_num) if trial_num > 0 else 0.0
    print(
        f"[obj {args.obj}] Success rate over {trial_num} loaded trials "
        f"(starting from trial index {trial_start}): "
        f"{success_rate}/{trial_num} = {success_ratio:.2%}"
    )
    return {
        "obj_name": str(args.obj),
        "success_count": int(success_rate),
        "trial_count": int(trial_num),
        "trial_start": int(trial_start),
        "trials_path": trails_path,
        "interrupted": bool(object_interrupted),
    }
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="*",
        default="stanford_bunny2",
        help="Object name(s) to evaluate. Omit to auto-discover all object folders under --trails-input-root.",
    )
    ...
    parser.add_argument("--anti-tip-force-max", type=float, default=0.1, help="upper bound on the optimizer contact force used only for the anti-tip stabilizing torque")
    parser.add_argument("--anti-tip-torque-max", type=float, default=0.1, help="norm cap on the anti-tip stabilizing torque applied to the object in world frame")
    parser.add_argument("--stage1-torque-pd-kp-pos", type=float, default=0.0, help="P gain on the optimizer-predicted local position offset x_plus during the z-axis flip stage")
    parser.add_argument("--stage1-torque-pd-kp-ori", type=float, default=0.2, help="P gain on the optimizer-predicted local orientation offset x_plus during the z-axis flip stage")
    parser.add_argument("--stage1-torque-pd-kd-lin", type=float, default=0.0, help="D gain on local linear velocity tracking error v_plus - v during the z-axis flip stage")
    parser.add_argument("--stage1-torque-pd-kd-ang", type=float, default=0.08, help="D gain on local angular velocity tracking error v_plus - v during the z-axis flip stage")
    parser.add_argument("--stage1-torque-scale-min", type=float, default=0.9, help="lower clamp for the PD-based torque scale applied to optimizer torque during the z-axis flip stage")
    parser.add_argument("--stage1-torque-scale-max", type=float, default=1.2, help="upper clamp for the PD-based torque scale applied to optimizer torque during the z-axis flip stage")
    ...
    parser.add_argument("--low-err-coef-flag0", type=float, default=0.75, help="low_err_coef used when best_contact_torque_scale_flag=0; defaults to --low_err_coef")
    parser.add_argument("--low-err-coef-flag1", type=float, default=0.3, help="low_err_coef used when best_contact_torque_scale_flag=1; defaults to --low_err_coef")
    parser.add_argument("--upper-err-coef-flag0", type=float, default=0.75, help="upper_err_coef used when best_contact_torque_scale_flag=0; defaults to --upper_err_coef")
    parser.add_argument("--upper-err-coef-flag1", type=float, default=0.95, help="upper_err_coef used when best_contact_torque_scale_flag=1; defaults to --upper_err_coef")
    ...
    parser.add_argument("--show-goal-pose", type=_parse_bool_arg, default=False, help="whether to render the goal-pose axes and semi-transparent ghost object (true/false)")
    parser.add_argument("--test", type=_parse_bool_arg, default=True, help="when true, keep the arm fixed and directly apply the optimizer torque to the object (true/false)")
    parser.add_argument("--execute-best-force", type=_parse_bool_arg, default=True, help="when true, turn best_force_lam into an end-effector desired force target during real execution (true/false)")
    ...
    parser.add_argument("--svg-screenshot-dir", type=str, default="/home/lab423/scsp/scsp-robot/examples/mpc/franka/ik2/figs", help="Directory to save Isaac Gym SVG screenshots. Leave empty to disable periodic capture.")
    parser.add_argument("--svg-screenshot-interval", type=float, default=0.2, help="Simulation-time seconds between Isaac Gym SVG screenshots.")
    parser.add_argument("--svg-screenshot-width", type=int, default=1280, help="Width of Isaac Gym SVG screenshots in pixels.")
    parser.add_argument("--svg-screenshot-height", type=int, default=960, help="Height of Isaac Gym SVG screenshots in pixels.")
    parser.add_argument("--video-output-dir", type=str, default=DEFAULT_RECORD_OUTPUT_DIR, help="Base directory for per-trial recording folders. Each trial saves video.mp4 and traj.json together under <obj>/<NNN>/.")
    parser.add_argument("--video-fps", type=float, default=20.0, help="Output MP4 frame rate.")
    parser.add_argument("--video-width", type=int, default=1280, help="Width of Isaac Gym MP4 frames in pixels.")
    parser.add_argument("--video-height", type=int, default=960, help="Height of Isaac Gym MP4 frames in pixels.")
    parser.add_argument("--traj-output-dir", type=str, default=DEFAULT_RECORD_OUTPUT_DIR, help="Legacy fallback base directory for per-trial recording folders when --video-output-dir is empty.")
    parser.add_argument("--trails-input-root", type=str, default=DEFAULT_TRAILS_INPUT_ROOT, help="Root directory containing per-object trials.json/trails.json files used to set init/goal poses.")
    parser.add_argument("--print-object-pose", type=_parse_bool_arg, default=True, help="Whether to print the object xyz and quaternion during rollout (true/false).")
    parser.add_argument("--print-object-pose-interval", type=int, default=1, help="Print object pose every N rollout steps.")
    parser.add_argument("--success_pos_threshold", "--success-pos-threshold", dest="success_pos_threshold", type=float, default=0.02, help="Stop the rollout once object position error to the task target pose drops below this threshold.")
    parser.add_argument("--success_quat_threshold", "--success-quat-threshold", dest="success_quat_threshold", type=float, default=0.04, help="Stop the rollout once object quaternion error to the task target pose drops below this threshold.")
    parser.add_argument("--small-action-break-min-video-seconds", type=float, default=120.0, help="If MP4 recording has lasted at least this many real-time seconds, stop the current trial early and continue to the next trial.")
    parser.add_argument("--cartesian_stiffness", type=float, nargs="+", default=DEFAULT_CARTESIAN_STIFFNESS.tolist(), help="Cartesian stiffness gains; pass 1, 2, or 6 values. Default matches MuJoCo.")
    parser.add_argument("--cartesian_damping", type=float, nargs="+", default=None, help="Cartesian damping gains; pass 1, 2, or 6 values. Default uses critical damping from stiffness.")
    parser.add_argument("--effort-joint-damping", type=float, default=DEFAULT_EFFORT_JOINT_DAMPING, help="Passive damping applied to the 7 arm joints in Isaac effort mode.")
    parser.add_argument("--seed", "--init-rand-seed", dest="seed", type=int, default=15, help="Base random seed used to reproducibly sample init_xy_rand and init_obj_quat_rand.")
    parser.add_argument("--trial-start", type=int, default=3, help="first trail index to evaluate from the loaded trails.json/trials.json")
    parser.add_argument("--trial-count", type=int, default=20, help="number of consecutive loaded trail entries to run when explicitly specified")

    args = parser.parse_args()
    args.init_rand_seed = int(args.seed)
    args.set_axis_pose = _parse_pose_wxyz_arg(args.set_axis_pose, "--set_axis_pose")
    trial_count_overridden = any(
        arg == "--trial-count" or arg.startswith("--trial-count=")
    ...
    if trial_count_overridden and args.trial_count <= 0:
    ...
    if args.small_action_break_min_video_seconds < 0.0:
        raise ValueError(
            "small_action_break_min_video_seconds must be non-negative, "
            f"got {args.small_action_break_min_video_seconds}"
        )
    ...
    if float(args.stage1_torque_scale_max) < float(args.stage1_torque_scale_min):
        raise ValueError("stage1_torque_scale_max must be greater than or equal to stage1_torque_scale_min")

    object_names = _normalize_object_names(args.obj, args.trails_input_root)
    if object_names:
        print(
            f"Evaluating {len(object_names)} object(s) from "
            f"{os.path.abspath(args.trails_input_root)}: {', '.join(object_names)}"
        )
    ...
    for obj_name in object_names:
        object_args = argparse.Namespace(**vars(args))
        object_args.obj = obj_name
        print(f"\n===== Object: {obj_name} =====")
        try:
            summary = _run_trials_for_object(object_args, trial_count_overridden)
        except KeyboardInterrupt:
            print(f"\n[obj {obj_name}] Ctrl+C received. Stopping remaining objects.")
            break
        except Exception as exc:
            failed_objects.append({"obj_name": obj_name, "error": str(exc)})
            print(f"[obj {obj_name}] error: {exc}")
            continue
        object_summaries.append(summary)
        batch_success_count += summary["success_count"]
        batch_trial_count += summary["trial_count"]
        if summary["interrupted"]:
            break

    if len(object_summaries) > 1:
        print("\nBatch summary by object:")
        for summary in object_summaries:
            success_ratio = (
                float(summary["success_count"]) / float(summary["trial_count"])
                if summary["trial_count"] > 0
                else 0.0
            )
            print(
                f"[obj {summary['obj_name']}] "
                f"{summary['success_count']}/{summary['trial_count']} = {success_ratio:.2%}"
            )

    if failed_objects:
        print("\nFailed objects:")
        for failed in failed_objects:
            print(f"[obj {failed['obj_name']}] {failed['error']}")

    overall_success_ratio = (
        float(batch_success_count) / float(batch_trial_count)
        if batch_trial_count > 0
        else 0.0
    )
    print(
        f"Overall success rate over {batch_trial_count} trials across {len(object_summaries)} object(s): "
        f"{batch_success_count}/{batch_trial_count} = {overall_success_ratio:.2%}"
    )
