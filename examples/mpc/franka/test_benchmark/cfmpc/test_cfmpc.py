...
from examples.mpc.franka.test_benchmark.cfmpc.params_cfmpc import ExplicitMPCParams
...
DEFAULT_RECORD_OUTPUT_DIR = "/home/lab423/scsp/scsp-robot/outputs/videos_cmpc"
DEFAULT_TRAILS_INPUT_ROOT = "/home/lab423/scsp/scsp-robot/outputs/videos_panda"
...
def _load_trial_specs_from_trails(obj_name, trails_root):
    ...
    trails_path = os.path.join(trails_root, str(obj_name), "trails.json")
    if not os.path.exists(trails_path):
        raise FileNotFoundError(f"Missing trails.json for obj '{obj_name}': {trails_path}")
...
def main():
    ...
    parser.add_argument(
        ...
        help="Root directory containing per-object trails.json files used to set init/goal poses.",
    )
    parser.add_argument(
        "--print-object-pose",
        type=_parse_bool_arg,
    ...
    if trial_start >= len(all_trial_specs):
        raise ValueError(
            f"trial_start={trial_start} is out of range for {len(all_trial_specs)} trails loaded from {trails_path}"
        )
    ...
    print(f"Loaded {len(all_trial_specs)} trails from {trails_path}")
    ...
    for experiment_idx, trial_spec in enumerate(trial_specs):
        ...
        print(
            f"[trial {trial_count:03d}] source_trail={source_trial_id} "
    ...
    print(
        f"Success rate over {trial_num} loaded trails "
        f"(starting from trail index {trial_start}): "
