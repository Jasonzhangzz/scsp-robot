...
class BimanualPandaGrasper:
    ...
    def _detect_planner_contacts(self):
        if self.object_is_ghost:
            return self._reformat_planner_contacts([], [])
        mujoco.mj_forward(self.model, self.data)
    ...
    def run(self):
        ...
        left_stage2_rot, right_stage2_rot = self._build_stage2_tracking_rotations(
            left_fixed_rot,
            right_fixed_rot,
        )
        if bool(self.args.verbose):
            ...
            print("Stage 2: move first to attract_point_world = contact_points_world + outward_normal * planner_attract_offset")
            print(
                f"  Stage 2 cfg: upward_tilt={float(self.args.stage2_up_tilt):.3f} rad "
                f"attract_offset={float(self.args.planner_attract_offset):.4f} m"
            )
        ...
        if not stage2_ok:
            print("Attract stage reached its step limit; continuing to Stage 3 with the current pose.")

        if bool(self.args.verbose):
            print("Stage 3: approach the projected contact_points_world from the attract points")
        stage3_ok, _stage3_info = self._run_live_contact_plan_stage(
            "touch",
        ...
        if not stage3_ok:
            print("Touch stage reached its step limit; proceeding to the squeeze stage with the current pose.")
        ...
        squeeze_ok, squeeze_info = self._run_live_contact_plan_stage(
            ...
            apply_optimizer_object_torque=True,
        )
        ...
        if "applied_optimizer_torque_world" in squeeze_info:
            print(
                "Applied optimizer torque after bilateral contact:",
                np.array2string(np.asarray(squeeze_info["applied_optimizer_torque_world"], dtype=np.float64), precision=4),
            )
        if "optimizer_torque_application_point_world" in squeeze_info:
            print(
                "Optimizer torque application point world:",
                np.array2string(np.asarray(squeeze_info["optimizer_torque_application_point_world"], dtype=np.float64), precision=4),
            )
        ...
        def lift_success(info):
            ...
            if modeled_normal_forces.size == 2:
                contact_force_ok = bool(np.all(modeled_normal_forces >= 0.1 * desired_normal_forces))
            else:
                contact_force_ok = (
        ...
        lift_ok, lift_info = self._run_live_contact_plan_stage(
            "lift",
        ...
        if "applied_optimizer_torque_world" in lift_info:
            print(
                "Applied optimizer torque during lift:",
                np.array2string(np.asarray(lift_info["applied_optimizer_torque_world"], dtype=np.float64), precision=4),
            )
        if "optimizer_torque_application_point_world" in lift_info:
            print(
                "Optimizer torque application point world during lift:",
                np.array2string(np.asarray(lift_info["optimizer_torque_application_point_world"], dtype=np.float64), precision=4),
            )
...
def build_argparser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Seconds between automatic screenshots. Use 0 to disable.",
    )
