...
class ExplicitMPCParams2:
    ...
    def init_cost_fns(self):
        ...
        contact_cost = cs.sumsqr(x[0:3] - x[10:])
        ...
        reject_distance = cs.sumsqr(x[0:2] - x[10:12]) + 1e-3
        obstacle_cost = cs.if_else(reject_distance < self.reject_dis, 1 / (reject_distance), 0.0)
        attract_cost = self.attract_coef * virtual_point_cost + self.reject_coef * obstacle_cost  # 世界坐标系
        ...
        base_cost = (1 - verify_cost_param) * attract_cost + self.contact_coef * verify_cost_param * (contact_cost_param * contact_cost + (1-contact_cost_param) * contact_point_cost)
 
        # final_cost = 500 * position_cost + 5.0 * quaternion_cost * curr_ori_coef
