...
try:
    from project_point2 import CabinetProjectionPoint
except ImportError:
    from planning.project_point2 import CabinetProjectionPoint
...
class LambdaContactControlOptimizer:
    def __init__(self, mesh_path, obj_mass=0.01, arm_friction=0.9, 
                 ...
                 sample_bounds=None, handle_link_name="drawer_handle_top"):
        # 系统参数
        self.m = obj_mass
        ...
        requested_handle_link = str(handle_link_name).strip().lower()
        if requested_handle_link in {"top", "handle_top"}:
            requested_handle_link = "drawer_handle_top"
        elif requested_handle_link in {"bottom", "handle_bottom"}:
            requested_handle_link = "drawer_handle_bottom"
        self.handle_link_name = requested_handle_link
        mesh_path_obj = Path(mesh_path)
        if mesh_path_obj.suffix.lower() == ".urdf":
            self.pp = CabinetProjectionPoint(
                mesh_path_obj,
                part=self.handle_link_name,
                use_collision=False,
                scale_factors=scale_factors,
            )
            # print("aaaaaaaaaaa")
        else:
            self.pp = ProjectionPoint(mesh_path_obj, scale_factors)

        self.sample_num = sample_num
        self.sample_bounds = sample_bounds
        self.sampling_frame = self._sample_contact_frames()
    ...
    def _resolve_candidate_indices(self):
        candidate_indices = np.arange(len(self.pp.vertices), dtype=int)
        if self.sample_bounds is None:
            return candidate_indices

        vertices = np.asarray(self.pp.vertices)
        mask = np.ones(vertices.shape[0], dtype=bool)
        for axis, axis_idx in zip(("x", "y", "z"), range(3)):
            bounds = self.sample_bounds.get(axis)
            if bounds is None:
                continue
            lower, upper = bounds
            if lower is not None:
                mask &= vertices[:, axis_idx] >= lower
            if upper is not None:
                mask &= vertices[:, axis_idx] <= upper

        candidate_indices = np.where(mask)[0]
        if candidate_indices.size == 0:
            raise ValueError(f"No sampled vertices fall inside local bounds: {self.sample_bounds}")
        return candidate_indices

    def _sample_contact_frames(self):
        candidate_indices = self._resolve_candidate_indices()
        candidate_count = candidate_indices.shape[0]
        sample_count = min(self.sample_num, candidate_count)

        if sample_count == candidate_count:
            sample_indices = candidate_indices
        else:
            candidate_vertices = np.asarray(self.pp.vertices)[candidate_indices]
            selected_local = np.zeros(sample_count, dtype=int)
            selected_local[0] = np.random.randint(candidate_count)
            distances = np.full(candidate_count, np.inf)
            for i in range(1, sample_count):
                prev_vertex = candidate_vertices[selected_local[i - 1]]
                new_distances = np.linalg.norm(candidate_vertices - prev_vertex, axis=1)
                distances = np.minimum(distances, new_distances)
                selected_local[i] = np.argmax(distances)
            sample_indices = candidate_indices[selected_local]

        frames = {
            'points': np.asarray(self.pp.vertices)[sample_indices],
            'normals': np.zeros((len(sample_indices), 3)),
            'tangent1': np.zeros((len(sample_indices), 3)),
            'tangent2': np.zeros((len(sample_indices), 3))
        }
        for i, idx in enumerate(sample_indices):
            n, t1, t2 = self.pp.get_vertex_frame(idx)
            frames['normals'][i] = n
    ...
    def _precompile_optimization_function(self):
        ...
        phi_vec = opti.parameter(4 * self.max_contacts)
        ...
        # 使用 explicit_model.init_model 的显式接触模型形式:
        # contact_force = fmax(-sigma * (jac_mat @ Q_inv @ b + phi_vec), 0)
        sigma = opti.parameter()
        opti.set_value(sigma, self.K_contact * self.h)

        # 计算接触力
        Q_inv_b = cs.MX(self.Q_inv) @ b
        contact_force = cs.fmax(-sigma * (J_tilde @ Q_inv_b + phi_vec), 0)
        ...
        self.optimization_fn = opti.to_function(
            ...
            [x_d, current_x, J_tilde, tau_o_np, n_arm, t1, t2, p_arm, curr_ori_coef, phi_vec],
    ...
    def _format_phi_vec(self, phi_vec):
        expected_dim = 4 * self.max_contacts
        if phi_vec is None:
            return np.zeros(expected_dim, dtype=np.float32)

        phi_vec = np.asarray(phi_vec, dtype=np.float32).reshape(-1)
        if phi_vec.shape[0] == expected_dim:
            return phi_vec

        formatted_phi = np.zeros(expected_dim, dtype=np.float32)
        copy_dim = min(expected_dim, phi_vec.shape[0])
        formatted_phi[:copy_dim] = phi_vec[:copy_dim]
        return formatted_phi
    
    def optimize_control_input(self, x_d, current_x, tau_o, p_arm=None, phi_vec=None):
        """优化控制输入 - 更新接口"""
        if p_arm is None:
            p_arm = np.array([-1, 0, 0])
        phi_vec = self._format_phi_vec(phi_vec)
    ...
    def choose_contact_points(self, x_d, current_x, tau_o, visible_face_idx, phi_vec=None):
