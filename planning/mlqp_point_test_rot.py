...
class LambdaContactControlOptimizer:
    def __init__(self, mesh_path, obj_mass=0.01, arm_friction=0.9,
        ...
        self.use_vertices = bool(use_vertices)

        self.sample_num = sample_num
        self.face_centers = np.asarray(self.pp.scaled_mesh.vertices[self.pp.scaled_mesh.faces].mean(axis=1), dtype=np.float32)
        ...
        if self.use_vertices:
            ...
            self.sampling_frame = self._sample_face_centers_with_normals(num_samples=self.sample_num)
        ...
        # Stage 1 ignores translation and only aligns the object z axis.
        # Stage 2 reintroduces translation and full quaternion refinement.
        self.pos_coef = pos_coef
        self.ori_coef = ori_coef
        self.stage1_default_lam_upper_bound = 1.2
        self.stage2_default_lam_upper_bound = 0.1
        self.point_idx = np.arange(self.sample_point.shape[0])
        self.init_utils()
    ...
    def _project_point_to_contact_proxy(self, p_arm):
        ...
        if self.face_center_tree is None:
            raise ValueError("Face-center KD-tree is not initialized.")

        _, nearest_idx = self.face_center_tree.query(p_arm.reshape(1, 3), k=1)
        closest_idx = int(np.asarray(nearest_idx).reshape(-1)[0])
        p_obj_local = np.asarray(self.face_centers[closest_idx], dtype=np.float32)
        normal_obj_local = np.asarray(self.face_normals[closest_idx], dtype=np.float32)
        n, t1, t2 = self._build_frame_from_normal(-normal_obj_local)
        return closest_idx, n, t1, t2, p_obj_local, normal_obj_local
    ...
    def _build_solver_bundle(self, use_full_pose_objective, default_lam_upper_bound):
        ...
        orientation_error = _quaternion_alignment_error(x_plus[3:7], x_d[3:7])
        objective = (
        ...
        if contact_torque_world is not None and torque_norm_upper_bound is not None:
            ...
            if torque_norm > max(torque_norm_upper_bound, 1e-8):
                contact_torque_world = (
                    ...
                    * (torque_norm_upper_bound / torque_norm)
                ).astype(np.float32)
        if contact_torque_world is not None:
    ...
    def choose_contact_points(self, x_d, current_x, tau_o, visible_face_idx, use_full_pose_objective=False):
        ...
        for i, idx in enumerate(visible_face_idx):
            # start_t = time.time()
            lam_arm, x_plus_opt, _, objective_value, _ = self._solve_once(
                x_d=x_d,
                current_x=current_x,
                tau_o=tau_o,
            ...
            pos_buffer.append(x_plus_opt[3:])
            force_buffer.append(lam_arm)
    ...
    def _solve_once(
        ...
        opti.set_value(bundle["curr_ori_coef_param"], np.asarray(curr_ori_coef, dtype=np.float64).reshape(1))
        lam_upper_bound = float(max(lam_upper_bound, 1e-3))
