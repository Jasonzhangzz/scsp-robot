...
class MPCExplicit:
    ...
    def _build_cost_fns(self):
        ...
        robot_contact_force_map_flat = cs.SX.sym("robot_contact_force_map_flat", 3 * self.param_.max_ncon_ * 4)
        robot_contact_torque_map_flat = cs.SX.sym("robot_contact_torque_map_flat", 3 * self.param_.max_ncon_ * 4)
        ...
        attract_cost_1 = self.param_.attract_coef * virtual_point_cost_1 + self.param_.reject_coef * reject_cost_1
        attract_cost_2 = self.param_.attract_coef * virtual_point_cost_2 + self.param_.reject_coef * reject_cost_2

        base_cost_1 = (1.0 - verify_cost_param_1) * attract_cost_1 + self.param_.contact_coef * verify_cost_param_1 * (
            self.param_.contact_cost_param * contact_cost_1
            + (1.0 - self.param_.contact_cost_param) * contact_point_cost_1
        )
        base_cost_2 = (1.0 - verify_cost_param_2) * attract_cost_2 + self.param_.contact_coef * verify_cost_param_2 * (
        ...
        robot_contact_force_map = cs.reshape(robot_contact_force_map_flat, 3, self.param_.max_ncon_ * 4)
        robot_contact_torque_map = cs.reshape(robot_contact_torque_map_flat, 3, self.param_.max_ncon_ * 4)
        ...
        final_cost_1 = (1.0 - verify_cost_param_1) * terminal_virtual_cost_1 + verify_cost_param_1 * terminal_contact_cost_1
        final_cost_2 = (1.0 - verify_cost_param_2) * terminal_virtual_cost_2 + verify_cost_param_2 * terminal_contact_cost_2
        final_cost = 10.0 * 500.0 * (final_cost_1 + final_cost_2)

        path_cost_fn = cs.Function("path_cost_fn_bigrasp", [x, u, cost_params], [path_cost])
        final_cost_fn = cs.Function("final_cost_fn_bigrasp", [x, cost_params], [final_cost])
    ...
    def _build_cost_param_vector(
        ...
        if desired_force_world is None:
            ...
            desired_force_world = np.asarray(desired_force_world, dtype=np.float64).reshape(3)
        if desired_torque_world is None:
            desired_torque_world = np.zeros(3, dtype=np.float64)
        else:
            desired_torque_world = np.asarray(desired_torque_world, dtype=np.float64).reshape(3)
        if robot_contact_force_map is None:
            robot_contact_force_map = np.zeros((3, self.param_.max_ncon_ * 4), dtype=np.float64)
        else:
        ...
        if robot_contact_torque_map is None:
            robot_contact_torque_map = np.zeros((3, self.param_.max_ncon_ * 4), dtype=np.float64)
        else:
    ...
    def _rollout_ctrl_guess(self, ctrls, curr_x, phi_vec, jac_mat):
        ctrls = np.asarray(ctrls, dtype=np.float64).reshape(self.param_.mpc_horizon_, self.param_.n_cmd_)
        ...
        for uk in ctrls:
            pred_q = np.asarray(
                self.model.step_once_fn(qk, uk, phi_vec, jac_mat, self.param_.model_params).full()
            ).reshape(-1)
            w0.extend([uk, pred_q])
            rollout_q.append(pred_q)
            qk = pred_q

        x0 = np.concatenate([np.asarray(block, dtype=np.float64).reshape(-1) for block in w0], axis=0)
    ...
    def _extract_ctrl_guess(self, sol_guess):
        ...
        if ctrl_guess is not None:
            return np.asarray(ctrl_guess, dtype=np.float64).reshape(self.param_.mpc_horizon_, self.param_.n_cmd_)
    ...
    def _prepare_acados_guess(self, sol_guess, curr_x, phi_vec, jac_mat):
        ctrl_guess = self._extract_ctrl_guess(sol_guess)
        if ctrl_guess is None:
            ctrl_guess = np.zeros((self.param_.mpc_horizon_, self.param_.n_cmd_), dtype=np.float64)
    ...
    def _build_shifted_sol_guess(self, opt_u_traj, curr_x, phi_vec, jac_mat, backend, opt_cost, solve_status):
        opt_u_traj = np.asarray(opt_u_traj, dtype=np.float64).reshape(self.param_.mpc_horizon_, self.param_.n_cmd_)
        shifted_u_traj = self._shift_traj(opt_u_traj)
        shifted_x0, shifted_rollout_q = self._rollout_ctrl_guess(shifted_u_traj, curr_x, phi_vec, jac_mat)
    ...
    def _evaluate_rollout_cost(self, x_traj, u_traj, cost_params):
        x_traj = np.asarray(x_traj, dtype=np.float64).reshape(self.param_.mpc_horizon_ + 1, self.param_.n_qpos_)
        u_traj = np.asarray(u_traj, dtype=np.float64).reshape(self.param_.mpc_horizon_, self.param_.n_cmd_)
        cost_params = np.asarray(cost_params, dtype=np.float64).reshape(-1)

        total_cost = 0.0
        for stage_idx in range(self.param_.mpc_horizon_):
            total_cost += float(self.path_cost_fn(x_traj[stage_idx], u_traj[stage_idx], cost_params).full().item())
    ...
    def _plan_once_ipopt(self, curr_x, phi_vec, jac_mat, cost_params, sol_guess, requested_solver, fallback_reason=None):
    ...
    def _plan_once_acados(self, curr_x, phi_vec, jac_mat, cost_params, sol_guess):
        ...
        stage_param_vector = self._build_acados_param_vector(phi_vec, jac_mat, cost_params)
        ...
        try:
            ...
            solver.set(self.param_.mpc_horizon_, "x", warm_start["x_traj"][self.param_.mpc_horizon_])
            ...
            x_traj = np.asarray(
                [solver.get(stage_idx, "x") for stage_idx in range(self.param_.mpc_horizon_ + 1)],
                dtype=np.float64,
            ).reshape(self.param_.mpc_horizon_ + 1, self.param_.n_qpos_)
            u_traj = np.asarray(
                [solver.get(stage_idx, "u") for stage_idx in range(self.param_.mpc_horizon_)],
                dtype=np.float64,
    ...
    def plan_once(
        ...
        if requested_solver == "acados":
            ...
            acados_result = self._plan_once_acados(curr_x, phi_vec, jac_mat, cost_params, sol_guess)
    ...
    def _build_acados_solver_bundle(self):
        try:
            AcadosModel, AcadosOcp, AcadosOcpSolver, ACADOS_INFTY = _import_acados_template_symbols()
        except Exception as exc:
        ...
        model.disc_dyn_expr = self.model.step_once_fn(x, u, phi_vec, jac_mat, model_params)
        ...
        ocp.constraints.lbx_e = np.asarray(self.param_.mpc_q_lb_, dtype=np.float64).copy()
        ocp.constraints.ubx_e = np.asarray(self.param_.mpc_q_ub_, dtype=np.float64).copy()
        ...
        return {
            "backend": "acados",
            "solver": solver,
            "last_x_solution": np.zeros((self.param_.mpc_horizon_ + 1, self.param_.n_qpos_), dtype=np.float64),
            "last_u_solution": np.zeros((self.param_.mpc_horizon_, self.param_.n_cmd_), dtype=np.float64),
        }
