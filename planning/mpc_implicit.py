...
class MPCImplicit:
    ...
    def init_MPC(self):
        ...
        for k in range(self.param_.mpc_horizon_):
            ...
            g += [pred_q - qk]
            # slack_k = cs.SX.sym(f'slack_{k}', self.param_.n_qpos_)  # 新增松弛变量
            # w += [slack_k]
            # lbw += [-np.inf * np.ones(self.param_.n_qpos_)]  # 松弛变量无下界
            # ubw += [np.inf * np.ones(self.param_.n_qpos_)]   # 松弛变量无上界
            # w0 += [np.zeros(self.param_.n_qpos_)]            # 初始猜测为零

            # g += [pred_q - qk - slack_k]  # 原等式约束改为 g + s = 0
            # J += 1e4 * cs.dot(slack_k, slack_k)  # 惩罚松弛项
