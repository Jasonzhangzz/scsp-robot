...
class ExplicitModel:
    ...
    def init_model(self):
        ...
        # R = 1e-1 * cs.DM.eye(self.param_.max_ncon_ * 4)
        # # K = cs.inv(jac_mat@Q_inv@jac_mat.T + R)
        # K = cs.inv(R)
        # contact_force = -K @ (jac_mat @ Q_inv @ b + phi_vec)

        # calculate the contact term
        # Match the reference explicit model's smooth unilateral contact
        # response.  The acados wrapper reuses this exact CasADi dynamics.
        contact_force = -model_params @ (jac_mat @ Q_inv @ b + phi_vec) \
                        - 0.01 * model_params @ jac_mat @ Q_inv @ b / h
        beta = 100.0
